...
def process(stars):
    ...
    for s in stars:
        ...
        if len(nearby_stars_similar_pm) != 1:
            print("(%d) Skipping non-binary (%d):" % (sid, len(nearby_stars_similar_pm)))
            continue
...
current_id = 0
while True:
    stars_to_process = c.execute("SELECT %s FROM gaia WHERE source_id > %d LIMIT 1000" % (columns_to_fetch, current_id)).fetchall()

    if len(stars_to_process) == 0:
        break

    print("Processing 1000 (%d) stars from id %d" % (counter, current_id))
    counter = counter + 1000
    process(stars_to_process)
