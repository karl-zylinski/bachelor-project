...
for dec_entry in os.listdir(db_folder):
    dec_folder = "%s/%s" % (db_folder, dec_entry)
    if not os.path.isdir(dec_folder) or not dec_entry.isnumeric():
        continue

    dec = int(dec_entry)

    for ra_entry in os.listdir(dec_folder):
        dec_ra_folder = "%s/%s" % (dec_folder, ra_entry)
        
        if not os.path.isdir(dec_ra_folder) or not ra_entry.isnumeric():
            continue

        ra = int(ra_entry)

        for db in os.listdir(dec_ra_folder):
            if not db.endswith(".db"):
                continue

            db_filename = "%s/%s" % (dec_ra_folder, db)
