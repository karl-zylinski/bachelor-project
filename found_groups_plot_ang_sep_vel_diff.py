...
for g in cg["groups"]:
    ...
    d = (g["stars"][0][i_distance] + g["stars"][1][i_distance])/2

    pos1 = [x1, y1, z1]
    pos2 = [x2, y2, z2]
    sep = vec3.mag(vec3.sub(pos2, pos1)) # pc
    ang_sep_as = (sep / d)*conv.rad_to_arcsec # pc/pc = rad -> rad to arc sec
    v1 = [vx1, vy1, vz1]
    v2 = [vx2, vy2, vz2]
    vel_diff = vec3.mag(vec3.sub(v2, v1))

    seps.append(ang_sep_as)
    vel_diffs.append(vel_diff)

plt.plot(seps, vel_diffs, ".", markersize=1)
plt.xlabel("Sepration (arc seconds)")
